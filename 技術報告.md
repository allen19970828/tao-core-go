...
最終驗證結果：所有單元測試、race test 與程序級 E2E 通過；整體測試覆蓋率為 **60.3%**，HTTP handler 覆蓋率為 **90.0%**；govulncheck 顯示可到達漏洞 **0**；gosec 無 findings；PostgreSQL migration/schema verify 與 Linux 靜態建置成功。
...
- `.dockerignore` 排除 `.env`、本機資料庫、backup、Git metadata 與測試產物，避免進入 builder context/cache。
- Container healthcheck 改用具資料庫檢查的 `/ready`。
...
PostgreSQL 16 migration + schema verify
Process-level HTTP security/E2E acceptance
Docker build
```
...
cmd/migrate              0.0%  (由程序級 PostgreSQL integration 覆蓋)
cmd/server              21.9%
internal/database       75.0%
...
以 release mode 啟動真實 binary，並在 SQLite 隔離資料庫與 CI PostgreSQL 16 兩種環境執行黑箱驗收：
...
| Security headers | 存在 |
| Student 存取 ADMIN route | 403 |
| Webhook loopback SSRF | 400 |
| 含 SVG 的 QTI ZIP | 拒絕 |
| 跨使用者 session 查詢 | 404 |
| 開始、作答、監考、交卷、重複交卷 | PASS |
| ADMIN analytics 與 CSV export | PASS |
...
- GitHub Actions 已實際完成 PostgreSQL 16 migration、schema verify、程序級 E2E 與 Docker image build。
...
### 10.7 PostgreSQL 高併發情境

CI 已加入 PostgreSQL 16 service，會執行 migration、必要 unique index schema verify 與真實 HTTP E2E。現階段仍未建立多節點、長時間 soak test 與資料庫故障注入；大規模上線前應另行驗證 connection pool、lock contention、failover 與 concurrent submit 壓力。
...
| Migration/schema verify | `cmd/migrate/main.go`、`internal/database/database.go` |
...
| Staging/E2E | `tests/e2e/e2e_test.go`、`scripts/*.sh`、`docs/STAGING_RUNBOOK.md` |
...
本次已提供 PostgreSQL 備份、migration/schema verify、staging readiness、黑箱 E2E 與具確認鎖的 restore 工具；GitHub Actions 亦已對 PostgreSQL 16 與 Docker build 執行驗證。正式上線前仍必須依 `docs/STAGING_RUNBOOK.md` 在實際 staging 配置正式等級 secrets，完成 LTI/Webhook 外部整合驗收及人工 reviewer 核准。上線後則應持續追蹤依賴漏洞、集中式 rate limit、durable event delivery 與 key rotation 自動化。
